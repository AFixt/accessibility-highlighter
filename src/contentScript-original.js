 ...
 */
const PROHIBITED_TABLE_SUMMARIES = [
  'combobox',
  'Layout',
  'for layout',
  'layout table',
...
function runAccessibilityChecks() {
  ...
  for (const element of imageElements) {
    ...
    if (PROHIBITED_ALT_VALUES.includes(altValue.toLowerCase())) {
      console.log(element);
      overlay.call(
        element,
        'overlay',
  ...
  for (const element of linkElementsWithBadText) {
    ...
    if (PROHIBITED_LINK_TEXT.includes(linkText.toLowerCase())) {
      console.log(element);
      overlay.call(
        element,
        'overlay',
  ...
  for (const element of tablesWithSummary) {
    ...
    if (PROHIBITED_TABLE_SUMMARIES.some(badSummary =>
